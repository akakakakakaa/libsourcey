    ...
    , _context(_audioModule)
{
    ...
    _stream.attachSource(_encoder, true);
    _stream.emitter += packetSlot(_audioModule.get(), &wrtc::AudioPacketModule::onAudioCaptured);
}
...
void Signaler::onPeerConnected(smpl::Peer& peer)
{
    ...
    webrtc::MediaStreamInterface* stream = conn->createMediaStream();
    stream->AddTrack(_context.factory->CreateAudioTrack(wrtc::kAudioLabel, _context.factory->CreateAudioSource(nullptr)));
    auto oparams = _encoder->video()->oparams;
    auto source = new wrtc::VideoPacketSource(oparams.width, oparams.height, oparams.fps, cricket::FOURCC_I420);
    source->setPacketSource(&_stream.emitter);
    stream->AddTrack(_context.factory->CreateVideoTrack(wrtc::kVideoLabel, _context.factory->CreateVideoSource(source, nullptr)));
